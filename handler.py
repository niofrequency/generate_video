...
def download_file_from_url(url, output_path):
    """URL에서 파일을 다운로드하는 함수"""
    try:
        # wget을 사용하여 파일 다운로드 (timeout 60초 추가)
...
def handler(job):
    ...
    temp_workspace = f"/tmp/{task_id}"

    # 이미지 입력 처리 (image_path, image_url, image_base64 중 하나만 사용)
    image_path = None
    if "image_path" in job_input:
        image_path = process_input(job_input["image_path"], temp_workspace, "input_image.jpg", "path")
    elif "image_url" in job_input:
        image_path = process_input(job_input["image_url"], temp_workspace, "input_image.jpg", "url")
    elif "image_base64" in job_input:
        image_path = process_input(job_input["image_base64"], temp_workspace, "input_image.jpg", "base64")
    else:
        # 기본값 사용
    ...
    if "end_image_path" in job_input:
        end_image_path_local = process_input(job_input["end_image_path"], temp_workspace, "end_image.jpg", "path")
    elif "end_image_url" in job_input:
        end_image_path_local = process_input(job_input["end_image_url"], temp_workspace, "end_image.jpg", "url")
    elif "end_image_base64" in job_input:
        end_image_path_local = process_input(job_input["end_image_base64"], temp_workspace, "end_image.jpg", "base64")
    ...
    # Default 값들을 .get()으로 안전하게 할당 (KeyError 방지)
    ...
    if "244" in prompt: prompt["244"]["inputs"]["image"] = image_path
    if "541" in prompt: prompt["541"]["inputs"]["num_frames"] = length
    ...
    if "220" in prompt: prompt["220"]["inputs"]["seed"] = seed
    if "540" in prompt:
